...
# can calculate the maximum of each excursion.  Thus we need this function to break each
...
# Return an extended range around a given index.
def find_extended_ranges(indices, max_size, range_size=100):
    range_all = {}
    for (col, srs) in indices.iteritems():
        range = []
        for (i, index) in enumerate(srs):
            start = max(0, index-range_size)
            end   = min(max_size, index+range_size)
...
def main():
    ...
    #data.to_pickle('./data.out')
    #with open('./fields.out', 'wb') as pkl_file:
    #    pickle.dump(fields, pkl_file)
    #data = pd.read_pickle('./data.out')
    #with open('./fields.out', 'rb') as pkl_file:
    #    fields = pickle.load(pkl_file)
    
    """for row in data_stdev.iterrows():
        print row"""
    #col = '_VALDES-PM1:UTKV'
    #print data_stdev.info(verbose=True)
    #print data[col][::100]#[data_stdev[col][::100] > 1.0]
    #print data_stdev[col][data_stdev[col] > 0.12]
    #print data[col][data_stdev[col] > 0.12]
    
    groups = find_threshold_ranges(data_stdev, fields, threshold=thresh)
    (maxima, indices) = find_local_maxima(data_stdev, fields, groups)
    #index_ranges = find_extended_ranges(indices, len(data))
    
    # Output key--value pairs:  (timestamp),(field header) \t (index of stdev peak),(stdev peak)
    for c in fields:
        if not indices.has_key(c): continue
        for idx in indices[c]:
            print repr(data['Timestamp'][idx]) + ',' + repr(c) + '\t' + \
                  repr(idx) + ',' + repr(data_stdev[c][idx])
    
    """
    #plt.plot(data['_VALDES-PM1:UTKV'])
    plt.plot(data_stdev[col])
    #plt.plot(data_stdev[col][data_stdev[col] > 0.12])
    for i in index_ranges[col]:
        plt.plot(linspace(i[0], i[1]-1, 200), data_stdev[col][i[0]:i[1]], 'r--')
    plt.show()
    """
